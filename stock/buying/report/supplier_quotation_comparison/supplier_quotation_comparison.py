...
def get_data(filters):
    ...
    if filters.get("request_for_quotation"):
        query = query.where(
            sq_item.request_for_quotation == filters.get("request_for_quotation")
        )
...
def prepare_data(supplier_quotation_data, filters):
    ...
    group_by_field = (
        "supplier_name"
        if filters.get("group_by") == "Group by Supplier"
        else "item_code"
    )
    ...
    for data in supplier_quotation_data:
        ...
        row = {
            "item_code": (
                "" if group_by_field == "item_code" else data.get("item_code")
            ),  # leave blank if group by field
            "supplier_name": (
                "" if group_by_field == "supplier_name" else data.get("supplier_name")
            ),
    ...
    for group in groups:
        ...
        )  # Add item/supplier name in first group row
...
def prepare_chart_data(suppliers, qty_list, supplier_qty_price_map):
    ...
    currency_symbol = frappe.db.get_value(
        "Currency", frappe.db.get_default("currency"), "symbol"
    )
    for qty in qty_list:
        datapoints = {
