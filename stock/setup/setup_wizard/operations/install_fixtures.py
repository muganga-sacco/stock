...
def install(country=None):
    ...
    base_path = frappe.get_app_path("stock", "stock", "doctype")
...
def add_uom_data():
    ...
    uoms = json.loads(
        open(
            frappe.get_app_path("stock", "setup", "setup_wizard", "data", "uom_data.json")
        ).read()
    )
    for d in uoms:
    ...
    uom_conversions = json.loads(
        open(
            frappe.get_app_path(
                "stock", "setup", "setup_wizard", "data", "uom_conversion_data.json"
            )
        ).read()
    )
