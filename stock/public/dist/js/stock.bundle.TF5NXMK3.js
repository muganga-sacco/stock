(() => {
  ...
  // ../stock/stock/public/js/utils.js
  frappe.provide('stock');
  ...
  $.extend(stock, {
    get_currency: function (company) {
      ...
      if (company)
        return (
          ((_a = frappe.get_doc(':Company', company)) == null
            ? void 0
            : _a.default_currency) || frappe.boot.sysdefaults.currency
        );
  ...
  $.extend(stock.utils, {
    ...
    add_indicator_for_multicompany: function (frm, info) {
      ...
      var indicator = $(
        '<div class="flex-column col-xs-6"><div style="margin-top:10px"><h6>' +
          info.company +
          '</h6></div><div class="badge-link small" style="margin-bottom:10px"><span class="indicator blue">Annual Billing: ' +
          format_currency(info.billing_this_year, info.currency) +
          '</span></div><div class="badge-link small" style="margin-bottom:10px"><span class="indicator ' +
          color +
          '">Total Unpaid: ' +
      ...
      if (info.loyalty_points) {
        $(
          '<div class="badge-link small" style="margin-bottom:10px"><span class="indicator blue">Loyalty Points: ' +
            info.loyalty_points +
            '</span></div>'
        ).appendTo(indicator);
      }
    ...
    pick_serial_and_batch_bundle(
      ...
      frappe.db
        .get_value('Item', item_row.item_code, [
          'has_batch_no',
          'has_serial_no',
        ])
        .then((r) => {
          ...
          frappe.require(
            ...
            function () {
              new stock.SerialBatchPackageSelector(frm, item_row, (r2) => {
                if (r2) {
                  let update_values = {
              ...
              callback: function (data) {
                ...
                if (hold_statuses.includes(frm.doc.status)) {
                  ...
                  frm.dashboard.set_headline_alert(
                    '<div class="row"><div class="col-xs-12"><span class="indicator whitespace-nowrap ' +
                      message.indicator +
                      '"><span>' +
                      message.msg +
                      '</span></span> </div></div>'
  ...
  function attach_selector_button(
    inner_text,
    append_loction,
    context,
    grid_row
  ...
  // ../stock/stock/public/js/queries.js
  ...
  stock.queries.setup_warehouse_query = function (frm) {
    frm.set_query('warehouse', 'items', function (doc, cdt, cdn) {
      ...
      if (row.item_code) {
        $.extend(filters, {
          query: 'stock.controllers.queries.warehouse_query',
        });
  ...
  // ../stock/stock/public/js/sms_manager.js
  stock.SMSManager = function SMSManager(doc) {
    ...
    this.setup = function () {
      var default_msg = {
        Lead: '',
        Opportunity:
          'Your enquiry has been logged into the system. Ref No: ' + doc.name,
        Quotation:
        ...
        'Sales Invoice':
          ...
          (doc.po_no ? ' for your PO: ' + doc.po_no : ''),
        'Material Request':
          'Material Request ' + doc.name + ' has been raised in the system',
        'Purchase Order':
      ...
      if (
        in_list(['Sales Order', 'Delivery Note', 'Sales Invoice'], doc.doctype)
      )
        this.show(
          doc.contact_person,
          'Customer',
    ...
    this.get_contact_number = function (contact, ref_doctype, ref_name) {
      frappe.call({
        method:
          'frappe.core.doctype.sms_settings.sms_settings.get_contact_number',
        args: {
  ...
  // ../stock/stock/public/js/utils/party.js
  frappe.provide('stock.utils');
  var SALES_DOCTYPES = [
    'Quotation',
    'Sales Order',
  ...
  stock.utils.get_party_details = function (frm, method, args, callback) {
    if (!args) {
      if (
        ...
        (frm.doc.party_name &&
          in_list(['Quotation', 'Opportunity'], frm.doc.doctype))
      ) {
        let party_type = 'Customer';
        if (
          frm.doc.quotation_to &&
          in_list(['Lead', 'Prospect'], frm.doc.quotation_to)
        ) {
    ...
    if (frappe.meta.get_docfield(frm.doc.doctype, 'taxes')) {
      if (
        !stock.utils.validate_mandatory(
          frm,
  ...
  stock.utils.get_address_display = function (
    frm,
    address_field,
    display_field,
  ...
  stock.utils.set_taxes_from_address = function (
    frm,
  ...
  // ../stock/stock/public/js/controllers/stock_controller.js
  frappe.provide('stock.stock');
  stock.stock.StockController = class StockController extends (
    ...
    setup_posting_date_time_check() {
      frappe.ui.form.on(
        this.frm.doctype,
        'set_posting_date_and_time_read_only',
        function (frm) {
  ...
  // ../stock/stock/public/js/payment/payments.js
  stock.payments = class payments extends stock.stock.StockController {
    ...
    make_multimode_payment() {
      ...
      this.payments = frappe.model.add_child(
        this.frm.doc,
        'Multi Mode Payment',
        'payments'
      );
      this.payments.mode_of_payment =
        this.dialog.fields_dict.mode_of_payment.get_value();
      ...
      if (this.frm.doc.payments.length) {
        $.each(this.frm.doc.payments, function (index, data) {
          ...
          if (data.type == 'Cash' && data.amount == me.frm.doc.paid_amount) {
            me.idx = data.idx;
            me.selected_mode = $(me.$body).find(
              repl("input[idx='%(idx)s']", { idx: me.idx })
            );
    ...
    set_outstanding_amount() {
      this.selected_mode = $(this.$body).find(
        repl("input[idx='%(idx)s']", { idx: this.idx })
      );
      this.highlight_selected_row();
      this.payment_val = 0;
      if (
        this.frm.doc.outstanding_amount > 0 &&
        flt(this.selected_mode.val()) == 0
      ) {
        ...
        this.selected_mode.val(
          format_currency(this.payment_val, this.frm.doc.currency)
        );
    ...
    highlight_selected_row() {
      var selected_row = $(this.$body).find(
        repl(".pos-payment-row[idx='%(idx)s']", { idx: this.idx })
      );
      $(this.$body)
        .find('.pos-payment-row')
        .removeClass('selected-payment-mode');
    ...
    bind_numeric_keys_event() {
      ...
      $(this.$body)
        ...
        .click(function () {
          ...
          me.selected_mode.val(
            format_currency(me.payment_val, me.frm.doc.currency)
          );
      ...
      if (in_list(['change_amount', 'write_off_amount'], this.idx)) {
        ...
        if (me.idx == 'change_amount') {
          ...
          if (
            flt(value) == 0 &&
            update_write_off &&
            me.frm.doc.outstanding_amount > 0
          ) {
            value = flt(
    ...
    show_amounts() {
      ...
      $(this.$body)
        .find('.write_off_amount')
        .val(
          format_currency(this.frm.doc.write_off_amount, this.frm.doc.currency)
        );
      $(this.$body)
        .find('.paid_amount')
  ...
  // ../stock/stock/public/js/controllers/taxes_and_totals.js
  stock.taxes_and_totals = class TaxesAndTotals extends stock.payments {
    ...
    apply_pricing_rule_on_item(item) {
      ...
      if (
        in_list(['Sales Order', 'Quotation'], item.parenttype) &&
        item.blanket_order_rate
      ) {
      ...
      if (item.margin_type == 'Percentage') {
        ...
        item.rate_with_margin =
          flt(effective_item_rate) + flt(item.margin_rate_or_amount);
      }
      item.base_rate_with_margin =
        flt(item.rate_with_margin) * flt(this.frm.doc.conversion_rate);
      ...
      if (item.discount_amount) {
        item_rate = flt(
          item.rate_with_margin - item.discount_amount,
          precision('rate', item)
        );
        item.discount_percentage =
          (100 * flt(item.discount_amount)) / flt(item.rate_with_margin);
      }
    ...
    calculate_item_values() {
      ...
      if (!this.discount_amount_applied) {
        for (const item of this.frm._items || []) {
          ...
          item.qty =
            item.qty === void 0 ? (me.frm.doc.is_return ? -1 : 1) : item.qty;
          if (!(me.frm.doc.is_return || me.frm.doc.is_debit_note)) {
            ...
            if (!qty) {
              ...
              if (
                me.frm.doc.doctype !== 'Purchase Receipt' &&
                me.frm.doc.is_return === 1
              ) {
                qty = flt(item.qty);
              }
            }
    ...
    initialize_taxes() {
      ...
      $.each(this.frm.doc['taxes'] || [], function (i, tax) {
        ...
        if (!this.discount_amount_applied) {
          stock.accounts.taxes.validate_taxes_and_charges(
            tax.doctype,
            tax.name
          );
    ...
    fetch_round_off_accounts() {
      ...
      if (me.frm.doc.company) {
        frappe.call({
          method:
            'stock.controllers.taxes_and_totals.get_round_off_applicable_accounts',
          args: {
            company: me.frm.doc.company,
    ...
    determine_exclusive_rate() {
      ...
      $.each(me.frm._items || [], function (n, item) {
        ...
        $.each(me.frm.doc['taxes'] || [], function (i, tax) {
          var current_tax_fraction = me.get_current_tax_fraction(
            tax,
            item_tax_map
          );
          ...
          total_inclusive_tax_amount_per_qty +=
            inclusive_tax_amount_per_qty * flt(item.qty);
        });
        if (
          !me.discount_amount_applied &&
          item.qty &&
      ...
      if (cint(tax.included_in_print_rate)) {
        ...
        if (tax.charge_type == 'On Net Total') {
          ...
          current_tax_fraction =
            (tax_rate / 100) *
            this.frm.doc['taxes'][cint(tax.row_id) - 1]
              .tax_fraction_for_current_item;
    ...
    calculate_taxes() {
      ...
      $.each(this.frm._items || [], function (n, item) {
        var item_tax_map = me._load_item_tax_rate(item.item_tax_rate);
        $.each(me.frm.doc['taxes'] || [], function (i, tax) {
          var current_tax_amount = me.get_current_tax_amount(
            item,
            tax,
            item_tax_map
          );
          ...
          if (tax.category) {
            current_tax_amount =
              tax.category == 'Valuation' ? 0 : current_tax_amount;
          ...
          if (i == 0) {
            ...
            tax.grand_total_for_current_item = flt(
              me.frm.doc['taxes'][i - 1].grand_total_for_current_item +
                current_tax_amount
            );
          }
          if (n == me.frm._items.length - 1) {
            ...
            if (
              i == me.frm.doc['taxes'].length - 1 &&
              me.discount_amount_applied &&
              me.frm.doc.apply_discount_on == 'Grand Total' &&
              me.frm.doc.discount_amount
            ) {
              me.frm.doc.rounding_adjustment = flt(
                me.frm.doc.grand_total -
                  flt(me.frm.doc.discount_amount) -
                  tax.total,
    ...
    set_cumulative_total(row_idx, tax) {
      ...
      if (row_idx == 0) {
        tax.total = flt(
          this.frm.doc.net_total + tax_amount,
          precision('total', tax)
        );
      } else {
        tax.total = flt(
          this.frm.doc['taxes'][row_idx - 1].total + tax_amount,
          precision('total', tax)
        );
      }
    }
    ...
    set_item_wise_tax(item, tax, tax_rate, current_tax_amount) {
      ...
      let item_wise_tax_amount =
        current_tax_amount * this.frm.doc.conversion_rate;
      if (frappe.flags.round_row_wise_tax) {
      ...
      tax_detail[key] = [
        tax_rate,
        flt(item_wise_tax_amount, precision('base_tax_amount', tax)),
      ];
    }
    round_off_totals(tax) {
      if (
        frappe.flags.round_off_applicable_accounts.includes(tax.account_head)
      ) {
        tax.tax_amount = Math.round(tax.tax_amount);
        tax.tax_amount_after_discount_amount = Math.round(
    ...
    round_off_base_values(tax) {
      if (
        ...
        tax.base_tax_amount_after_discount_amount = Math.round(
      ...
      if (this.frm.doc['taxes'] && this.frm.doc['taxes'].length) {
        ...
        if (any_inclusive_tax) {
          ...
          if (
            diff &&
            Math.abs(diff) <=
              5 / Math.pow(10, precision('tax_amount', last_tax))
          ) {
    ...
    calculate_totals() {
      ...
      this.frm.doc.grand_total = flt(
        tax_count
          ? this.frm.doc['taxes'][tax_count - 1].total +
      ...
      if (
        ...
        this.frm.doc.taxes_and_charges_added =
          this.frm.doc.taxes_and_charges_deducted = 0;
        if (tax_count) {
          $.each(this.frm.doc['taxes'] || [], function (i, tax) {
            if (in_list(['Valuation and Total', 'Total'], tax.category)) {
              if (tax.add_deduct_tax == 'Add') {
                me.frm.doc.taxes_and_charges_added += flt(
                  tax.tax_amount_after_discount_amount
                );
              } else {
        ...
        this.frm.doc.base_grand_total = flt(
          this.frm.doc.taxes_and_charges_added ||
            this.frm.doc.taxes_and_charges_deducted
            ? flt(this.frm.doc.grand_total * this.frm.doc.conversion_rate)
            : this.frm.doc.base_net_total
        );
    ...
    apply_discount_amount() {
      ...
      if (this.frm.doc.discount_amount) {
        ...
        if (total_for_discount_amount) {
          $.each(this.frm._items || [], function (i, item) {
            distributed_amount =
              (flt(me.frm.doc.discount_amount) * item.net_amount) /
            ...
            if (
              (!(me.frm.doc.taxes || []).length ||
                total_for_discount_amount == me.frm.doc.net_total ||
                me.frm.doc.apply_discount_on == 'Net Total') &&
              i == (me.frm._items || []).length - 1
            ) {
              var discount_amount_loss = flt(
                me.frm.doc.net_total - net_total - me.frm.doc.discount_amount,
                precision('net_total')
              );
              item.net_amount = flt(
    ...
    get_total_for_discount_amount() {
      if (this.frm.doc.apply_discount_on == 'Net Total') {
        ...
        $.each(this.frm.doc['taxes'] || [], function (i, tax) {
          if (in_list(['Actual', 'On Item Quantity'], tax.charge_type)) {
            ...
            var actual_tax_amount =
              (flt(actual_taxes_dict[tax.row_id]) * flt(tax.rate)) / 100;
        ...
        return flt(
          this.frm.doc.grand_total - total_actual_tax,
          precision('grand_total')
        );
      }
    }
    calculate_total_advance(update_paid_amount) {
      ...
      this.frm.doc.total_advance = flt(
    ...
    is_internal_invoice() {
      if (
        ['Sales Invoice', 'Purchase Invoice'].includes(this.frm.doc.doctype)
      ) {
    ...
    set_default_payment(total_amount_to_pay, update_paid_amount) {
      ...
      if (
        this.frm.doc.is_pos &&
        (update_paid_amount === void 0 || update_paid_amount)
      ) {
        $.each(this.frm.doc['payments'] || [], function (index, data) {
    ...
    calculate_paid_amount() {
      ...
      this.frm.set_value(
        'paid_amount',
        flt(paid_amount, precision('paid_amount'))
      );
      this.frm.set_value(
        'base_paid_amount',
        flt(base_paid_amount, precision('base_paid_amount'))
      );
    }
  ...
  // ../stock/stock/public/js/controllers/transaction.js
  stock.TransactionController = class TransactionController extends (
    ...
    setup() {
      ...
      frappe.ui.form.on(
        ...
        function (frm) {
          ...
          var total = flt(
            frm.doc[frappe.model.scrub(frm.doc.apply_discount_on)]
          );
    ...
    onload() {
      ...
      if (
        ...
        let sbb_field = this.frm.get_docfield(
          'items',
          'serial_and_batch_bundle'
        );
        if (sbb_field) {
    ...
    onload_post_render() {
      ...
      if (
        ...
        !((_a = this.frm.doc.__onload) == null ? void 0 : _a.load_after_mapping)
      ) {
      ...
      if (
        frappe.meta.get_docfield(this.frm.doc.doctype + ' Item', 'item_code')
      ) {
                    ...
                    if (
                      ...
                      return frappe.db
                        ...
                        .then((r2) => {
                          if (
                            r2.message &&
                            (r2.message.has_batch_no ||
                              r2.message.has_serial_no)
                          ) {
    ...
    service_stop_date(frm, cdt, cdn) {
      ...
      if (child.service_stop_date) {
        ...
        if (stop_date < start_date) {
          ...
          frappe.model.set_value(cdt, cdn, 'service_stop_date', '');
          frappe.throw(
            __('Service Stop Date cannot be after Service End Date')
          );
        }
      }
    }
    sync_bundle_data() {
      ...
      if (this.frm.is_new() && doctypes.includes(this.frm.doc.doctype)) {
        const barcode_scanner = new stock.utils.BarcodeScanner({
          frm: this.frm,
        });
    ...
    apply_product_discount(args) {
      ...
      const exist_items = items.map(
        (row) => (row.item_code, row.pricing_rules)
      );
    ...
    make_quality_inspection() {
      ...
      const dialog = new frappe.ui.Dialog({
        ...
        primary_action: function () {
          ...
          frappe.call({
            method:
              'stock.controllers.stock_controller.make_quality_inspections',
            args: {
              doctype: me.frm.doc.doctype,
    ...
    blanket_order(doc, cdt, cdn) {
      ...
      if (
        ...
        (item.parenttype == 'Sales Order' ||
          item.parenttype == 'Purchase Order')
      ) {
        frappe.call({
    ...
    frappe.require(
      ...
      function () {
        if (in_list(['Sales Invoice', 'Delivery Note'], frm.doc.doctype)) {
          item_row.type_of_transaction = frm.doc.is_return
            ? 'Inward'
            : 'Outward';
        } else {
          item_row.type_of_transaction = frm.doc.is_return
  ...
  stock.apply_putaway_rule = (frm, purpose = null) => {
    ...
    frappe.call({
      method:
        'stock.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule',
      args: {
        doctype: frm.doctype,
        items: frm.doc.items,
        company: frm.doc.company,
  ...
  // frappe-html:/Users/abizeyimanavictor/msacco/apps/stock/stock/public/js/templates/item_selector.html
  frappe.templates[
    'item_selector'
...
`;
  ...
  // ../stock/stock/public/js/utils/item_selector.js
    ...
    add_item(item_code) {
      ...
      $.each(this.frm.doc.items || [], (i, d2) => {
        if (d2[this.item_field] === item_code) {
          ...
          frappe.show_alert({
            message: __('Added {0} ({1})', [item_code, d2.qty]),
            indicator: 'green',
          });
          added = true;
          return false;
  ...
  // ../stock/stock/public/js/help_links.js
  frappe.provide('frappe.help.help_links');
  var docsUrl = 'https://stock.com/docs/';
  ...
  // frappe-html:/Users/abizeyimanavictor/msacco/apps/stock/stock/public/js/templates/item_quick_entry.html
  frappe.templates[
    'item_quick_entry'
...
`;
  ...
  // ../stock/stock/public/js/utils/contact_address_quick_entry.js
  ...
  // ../stock/stock/public/js/utils/customer_quick_entry.js
  frappe.provide('frappe.ui.form');
  frappe.ui.form.CustomerQuickEntryForm =
    frappe.ui.form.ContactAddressQuickEntryForm;

  // ../stock/stock/public/js/utils/supplier_quick_entry.js
  frappe.provide('frappe.ui.form');
  frappe.ui.form.SupplierQuickEntryForm =
    frappe.ui.form.ContactAddressQuickEntryForm;

  // ../stock/stock/public/js/call_popup/call_popup.js
  var CallPopup = class {
      ...
      this.call_details = new frappe.ui.FieldGroup({
        fields: [
          ...
          },
          {
            ...
            click: () =>
              frappe.new_doc('Lead', { mobile_no: this.caller_number }),
            depends_on: () => !this.get_caller_name(),
          },
          {
            fieldtype: 'Column Break',
          ...
          },
          {
            ...
            click: () => {
              ...
              frappe
                .xcall(
                  'stock.telephony.doctype.call_log.call_log.add_call_summary_and_call_type',
                  {
        ...
        frm.add_custom_button(
          ...
          function () {
            frappe.call({
              type: 'GET',
              method:
                'stock.controllers.stock_controller.show_stock_ledger_preview',
              args: {
                company: frm.doc.company,
                doctype: frm.doc.doctype,
  ...
  stock.accounts.unreconcile_payment = {
    add_unreconcile_btn(frm) {
      if (frm.doc.docstatus == 1) {
        ...
        frappe.call({
          method:
            'stock.accounts.doctype.unreconcile_payment.unreconcile_payment.doc_has_references',
          args: {
            doctype: frm.doc.doctype,
            docname: frm.doc.name,
          },
          callback: function (r) {
            if (r.message) {
              frm.add_custom_button(
                __('UnReconcile'),
                function () {
                  stock.accounts.unreconcile_payment.build_unreconcile_dialog(
                    frm
                  );
                },
                __('Actions')
    ...
    build_unreconcile_dialog(frm) {
      if (
        ...
        frappe.call({
          method:
            'stock.accounts.doctype.unreconcile_payment.unreconcile_payment.get_linked_payments_for_doc',
          args: {
            company: frm.doc.company,
            doctype: frm.doc.doctype,
            docname: frm.doc.name,
            ...
            if (r.message) {
              ...
              let d = new frappe.ui.Dialog({
                ...
                primary_action(values) {
                  let selected_allocations = values.allocations.filter(
                    (x) => x.__checked
                  );
                  if (selected_allocations.length > 0) {
                    let selection_map =
                      stock.accounts.unreconcile_payment.build_selection_map(
                        frm,
                        selected_allocations
                      );
                    stock.accounts.unreconcile_payment.create_unreconcile_docs(
                      selection_map
                    );
                    d.hide();
                  } else {
    ...
    create_unreconcile_docs(selection_map) {
      frappe.call({
        method:
          'stock.accounts.doctype.unreconcile_payment.unreconcile_payment.create_unreconcile_doc_for_selection',
        args: {
  ...
  stock.utils.BarcodeScanner = class BarcodeScanner {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (!row) {
          if (this.dont_allow_new_row) {
            this.show_alert(
              __('Maximum quantity scanned for item {0}.', [item_code]),
              'red'
            );
            this.clean_up();
            reject();
        ...
        if (serial_no) {
          this.is_duplicate_serial_no(row, item_code, serial_no).then(
            (is_duplicate) => {
              if (!is_duplicate) {
    ...
    set_item(row, item_code, barcode, batch_no, serial_no) {
      return new Promise((resolve) => {
        ...
        if (this.prompt_qty) {
          frappe.prompt(
            __('Please enter quantity for item {0}', [item_code]),
            ({ value }) => {
    ...
    prepare_item_for_scan(row, item_code, barcode, batch_no, serial_no) {
      ...
      this.dialog = new frappe.ui.Dialog({
        ...
        fields: me.get_fields_for_dialog(
          row,
          item_code,
          barcode,
          batch_no,
      ...
      this.dialog.set_primary_action(__('Update'), () => {
        ...
        this.remaining_qty =
          flt(this.dialog.get_value('qty')) -
          flt(this.dialog.get_value('scanned_qty'));
        frappe.model.set_value(row.doctype, row.name, item_data);
        frappe.run_serially([
          () => this.set_barcode(row, this.dialog.get_value('barcode')),
          () =>
            this.set_serial_and_batch(
              row,
              item_code,
    ...
    add_child_for_remaining_qty(prev_row) {
      if (this.remaining_qty && this.remaining_qty > 0) {
        ...
        if (
          this.qty_field == 'qty' &&
          frappe.meta.has_field(row.doctype, 'stock_qty')
        ) {
    ...
    async set_serial_and_batch(row, item_code, serial_no, batch_no) {
      if (this.frm.is_new() || !row.serial_and_batch_bundle) {
        ...
        frappe.call({
          method:
            'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.update_serial_or_batch',
          args: {
    ...
    async create_serial_and_batch_bundle(row, entries, key) {
      frappe.call({
        method:
          'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.add_serial_batch_ledgers',
        args: {
          entries: entries[key],
          child_row: row,
          doc: this.frm.doc,
    ...
    show_scan_message(idx, exist = null, qty = 1) {
      if (exist) {
        this.show_alert(
          __('Row #{0}: Qty increased by {1}', [idx, qty]),
          'green'
        );
      } else {
    ...
    async is_duplicate_serial_no(row, item_code, serial_no) {
      ...
      const promise = new Promise((resolve, reject) => {
        if (this.frm.is_new() || !row.serial_and_batch_bundle) {
          is_duplicate = this.check_duplicate_serial_no_in_localstorage(
            item_code,
            serial_no
          );
          if (is_duplicate) {
    ...
    check_duplicate_serial_no_in_db(row, serial_no, response) {
      frappe.call({
        method:
          'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.is_duplicate_serial_no',
        args: {
          serial_no,
          bundle_id: row.serial_and_batch_bundle,
        },
    ...
    check_duplicate_serial_no_in_localstorage(item_code, serial_no) {
      ...
      if (entries[item_code]) {
        existing_row = entries[item_code].filter(
          (e) => e.serial_no === serial_no
        );
      }
    ...
    get_row_to_modify_on_scan(item_code, batch_no, uom, barcode) {
      ...
      return (
        this.items_table.find(matching_row) || this.get_existing_blank_row()
      );
    }
  ...
  // ../stock/stock/public/js/telephony.js
  ...
  // frappe-html:/Users/abizeyimanavictor/msacco/apps/stock/stock/public/js/templates/call_link.html
...
`;
  ...
  // ../stock/stock/public/js/bulk_transaction_processing.js
        ...
        () => {
          if (doc_name.length == 0) {
            frappe
              .call({
                method:
                  'stock.utilities.bulk_transaction.transaction_processing',
  ...
  // ../stock/stock/public/js/utils/crm_activities.js
  ...
  // frappe-html:/Users/abizeyimanavictor/msacco/apps/stock/stock/public/js/templates/crm_activities.html
...
.form-footer {
  ...
  // frappe-html:/Users/abizeyimanavictor/msacco/apps/stock/stock/public/js/templates/crm_notes.html
...
.notes-section .btn {
  ...
  // ../stock/stock/public/js/utils/landed_taxes_and_charges_common.js
  stock.landed_cost_taxes_and_charges = {
    setup_triggers: function (doctype) {
      frappe.ui.form.on(doctype, {
        refresh: function (frm) {
          let tax_field =
            frm.doc.doctype == 'Landed Cost Voucher'
              ? 'taxes'
              : 'additional_costs';
        ...
        set_base_amount: function (frm, cdt, cdn) {
          ...
          frappe.model.set_value(
            ...
            'base_amount',
            flt(
              flt(row.amount) * row.exchange_rate,
  ...
  // ../stock/stock/public/js/utils/sales_common.js
  frappe.provide('stock.selling');
  stock.sales_common = {
    setup_selling_controller: function () {
      stock.selling.SellingController = class SellingController extends (
        stock.TransactionController
      ) {
        setup() {
          super.setup();
          this.toggle_enable_for_stock_uom(
        ...
        setup_queries() {
          ...
          me.frm.set_query(
            ...
            stock.queries.address_query
          );
          me.frm.set_query(
            'dispatch_address_name',
            stock.queries.dispatch_address_query
          );
        ...
        customer_address() {
          ...
          stock.utils.set_taxes_from_address(
            this.frm,
            'customer_address',
            'customer_address',
            'shipping_address_name'
        ...
        shipping_address_name() {
          stock.utils.get_address_display(
            this.frm,
            'shipping_address_name',
            'shipping_address'
          );
        ...
        total_commission() {
          ...
          this.frm.set_value(
            'commission_rate',
            flt(
              (this.frm.doc.total_commission * 100) /
                amount_eligible_for_commission
            )
          );
        }
        allocated_percentage(doc, cdt, cdn) {
          ...
          if (sales_person.allocated_percentage) {
            ...
            sales_person.allocated_amount = flt(
              (this.frm.doc.amount_eligible_for_commission *
                sales_person.allocated_percentage) /
                100,
        ...
        toggle_editable_price_list_rate() {
          ...
          var editable_price_list_rate = cint(
            frappe.defaults.get_default('editable_price_list_rate')
          );
        ...
        calculate_commission() {
          if (
            !this.frm.fields_dict.commission_rate ||
            this.frm.doc.docstatus === 1
          )
            return;
          ...
          this.frm.doc.amount_eligible_for_commission =
            this.frm.doc.items.reduce(
              (sum, item) =>
                item.grant_commission ? sum + item.base_net_amount : sum,
              0
            );
          this.frm.doc.total_commission = flt(
            (this.frm.doc.amount_eligible_for_commission *
              this.frm.doc.commission_rate) /
              100,
        ...
        calculate_contribution() {
          var me = this;
          $.each(
            this.frm.doc.doctype.sales_team || [],
            function (i, sales_person) {
            ...
            .then((r) => {
              if (
                ...
                frappe.require(path, function () {
                  new stock.SerialBatchPackageSelector(me.frm, item, (r2) => {
                    if (r2) {
        ...
        project() {
          ...
          if (
            ...
            if (this.frm.doc.project) {
              frappe.call({
                method:
                  'stock.projects.doctype.project.project.get_cost_center_name',
  ...
  // ../stock/stock/public/js/controllers/buying.js
  frappe.provide('stock.buying');
  stock.buying = {
    setup_buying_controller: function () {
      stock.buying.BuyingController = class BuyingController extends (
        stock.TransactionController
      ) {
        setup() {
          super.setup();
              ...
              if (this.frm.doc.customer) {
                return {
                  ...
                  filters: {
                    ...
                    link_name: this.frm.doc.customer,
                  },
                };
              } else return stock.queries.company_address_query(this.frm.doc);
            });
          }
        }
        setup_queries(doc, cdt, cdn) {
          ...
          me.frm.set_query(
            'billing_address',
            stock.queries.company_address_query
          );
        ...
        refresh(doc) {
          ...
          this.frm.toggle_display(
            ...
            this.frm.doc.supplier_name &&
              this.frm.doc.supplier_name !== this.frm.doc.supplier
          );
          if (
            this.frm.doc.docstatus == 0 &&
            (this.frm.doctype === 'Purchase Order' ||
              this.frm.doctype === 'Material Request')
          ) {
        ...
        toggle_subcontracting_fields() {
          if (
            ...
            this.frm.fields_dict.supplied_items.grid.update_docfield_property(
              ...
              'read_only',
              this.frm.doc.__onload &&
                this.frm.doc.__onload.backflush_based_on === 'BOM'
            );
        ...
        qty(doc, cdt, cdn) {
          if (
            ...
            (doc.doctype == 'Purchase Invoice' &&
              (doc.update_stock || doc.is_return))
          ) {
        ...
        calculate_received_qty(doc, cdt, cdn) {
          ...
          if (
            !doc.is_return &&
            this.validate_negative_quantity(cdt, cdn, item, [
              'qty',
              'rejected_qty',
            ])
          ) {
          ...
          let received_qty = flt(
            item.qty + item.rejected_qty,
            precision('received_qty', item)
          );
          let received_stock_qty =
            flt(item.conversion_factor, precision('conversion_factor', item)) *
            flt(received_qty);
        ...
        manufacturer(doc, cdt, cdn) {
          ...
          if (row.manufacturer) {
            frappe.call({
              method:
                'stock.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no',
              args: {
        ...
        for (let item of frm.doc.items) {
          ...
          (r.message[0] || []).forEach(function (d) {
            if (
              ...
              if (qty > 0) {
                frappe.msgprint(
                  'Splitting ' + qty + ' units of ' + d.item_code
                );
                var newrow = frappe.model.add_child(
                  frm.doc,
                  item.doctype,
                  'items'
                );
    ...
    var dialog = new frappe.ui.Dialog({
      ...
      primary_action(args) {
        ...
        return frappe.call({
          type: 'GET',
          method:
            'stock.stock.doctype.packed_item.packed_item.get_items_from_product_bundle',
          args: {
            row: {
          ...
          callback: function (r) {
            ...
            if (!r.exc && r.message) {
              ...
              for (var i = 0; i < r.message.length; i++) {
                ...
                if (
                  frappe.meta.get_docfield(d.doctype, 'price_list_rate', d.name)
                ) {
  ...
  // ../stock/stock/public/js/utils/demo.js
  ...
  stock.financial_statements = {
    ...
    formatter: function (value, row, column, data, default_formatter, filter) {
      if (
        ...
        if (currentAnnualvalue == void 0) return 'NA';
        let annualGrowth = 0;
        if (lastAnnualValue == 0 && currentAnnualvalue > 0) {
          annualGrowth = 1;
        } else if (lastAnnualValue > 0) {
          annualGrowth =
            (currentAnnualvalue - lastAnnualValue) / lastAnnualValue;
        }
        ...
        if (column.colIndex >= 2) {
          ...
          if (currentAnnualvalue == void 0 || baseValue <= 0) return 'NA';
          const marginPercent =
            Math.round((currentAnnualvalue / baseValue) * 1e4) / 100;
      ...
      if (data && column.fieldname == 'account') {
        ...
        if (
          filter &&
          (filter == null ? void 0 : filter.text) &&
          (filter == null ? void 0 : filter.type) == 'contains'
        ) {
        ...
        if (data.account) {
          column.link_onclick =
            'stock.financial_statements.open_general_ledger(' +
            JSON.stringify(data) +
            ')';
        }
        column.is_tree = true;
    ...
    onload: function (report) {
      ...
      let fiscal_year = stock.utils.get_fiscal_year(
...
//# sourceMappingURL=stock.bundle.TF5NXMK3.js.map
