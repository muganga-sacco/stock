...
frappe.setup.on('before_load', function () {
  basic.setupslides_settings.map(frappe.setup.add_slide);
});

basic.setupslides_settings = [
  {
    // Organization
    name: 'organization',
    ...
    set_fy_dates: function (slide) {
      ...
      if (country) {
        let fy = basic.setupfiscal_years[country];
    ...
    bind_events: function (slide) {
      ...
      slide
        ...
        .on('change', function () {
          ...
          if (abbr.length > 10) {
            frappe.msgprint(
              __('Company Abbreviation cannot have more than 5 characters')
            );
