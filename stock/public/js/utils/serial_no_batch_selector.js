stock.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
	...
	create_serial_nos() {
		...
		frappe.call({
			method: 'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.create_serial_nos',
			args: {
				item_code: this.item.item_code,
	...
	download_csv_file() {
		...
		const method = `/api/method/stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(JSON.stringify(csvFileData))}`;
	...
	upload_csv_file() {
		...
		frappe.call({
			method: 'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.upload_csv_file',
			args: {
				item_code: this.item.item_code,
				file_path: file_path
			},
	...
	get_auto_data() {
		...
		if (qty) {
			frappe.call({
				method: 'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_auto_data',
				args: {
					item_code: this.item.item_code,
	...
	scan_barcode_data() {
		...
		if (scan_serial_no || scan_batch_no) {
			frappe.call({
				method: 'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.is_serial_batch_no_exists',
				args: {
					item_code: this.item.item_code,
	...
	update_serial_batch_no() {
		...
		if (scan_serial_no) {
			...
			if (!this.item.has_batch_no) {
				...
				frappe.call({
					method: 'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_batch_no_from_serial_no',
					args: {
						serial_no: scan_serial_no,
					},
					callback: (r) => {
	...
	render_data() {
		if (this.bundle) {
			frappe.call({
				method: 'stock.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_serial_batch_ledgers',
				args: {
					item_code: this.item.item_code,
					name: this.bundle,
