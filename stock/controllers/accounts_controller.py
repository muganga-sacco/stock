...
# from basic.setuputils import get_exchange_rate
...
# 	get_item_warehouse,
# )
from stock.utilities.regional import temporary_flag
...
class AccountsController(TransactionBase):
    ...
    def company_currency(self):
        if not hasattr(self, "__company_currency"):
            self.__company_currency = stock.get_company_currency(self.company)
    ...
    def ensure_supplier_is_not_blocked(self):
        is_supplier_payment = (
            self.doctype == "Payment Entry" and self.party_type == "Supplier"
        )
        ...
        if supplier and supplier_name and supplier.on_hold:
            if (is_buying_invoice and supplier.hold_type in ["All", "Invoices"]) or (
                ...
                if (
                    not supplier.release_date
                    or getdate(nowdate()) <= supplier.release_date
                ):
                    frappe.msgprint(
                        _("{0} is blocked so this transaction cannot proceed").format(
                            supplier_name
                        ),
                        raise_exception=1,
                    )
    ...
    def validate(self):
        ...
        if self.doctype in ["Purchase Invoice", "Sales Invoice"]:
            ...
            if cint(self.allocate_advances_automatically) and not cint(
    ...
    def _remove_references_in_unreconcile(self):
        ...
        rows = (
            ...
            .where(
                (upe.reference_doctype == self.doctype)
                & (upe.reference_name == self.name)
            )
            .run(as_dict=True)
        )
    ...
    def _remove_references_in_repost_doctypes(self):
        ...
        for _doctype in repost_doctypes:
            ...
            if rows:
                references_map = frappe._dict()
                for x in rows:
                    references_map.setdefault((x.parenttype, x.parent), []).append(
                        x.name
                    )
                ...
                for doc, rows in references_map.items():
                    ...
                    for row in rows:
                        if _doctype == "Repost Payment Ledger Items":
                            ...
                            repost_doc.remove(
                                repost_doc.get("vouchers", {"name": row})[0]
                            )
    ...
    def validate_return_against_account(self):
        if (
            ...
            cr_dr_account_field = (
                "debit_to" if self.doctype == "Sales Invoice" else "credit_to"
            )
            cr_dr_account_label = (
                "Debit To" if self.doctype == "Sales Invoice" else "Credit To"
            )
            cr_dr_account = self.get(cr_dr_account_field)
            if (
                frappe.get_value(self.doctype, self.return_against, cr_dr_account_field)
                != cr_dr_account
            ):
                frappe.throw(
                    _(
            ...
            if item.get("enable_deferred_revenue") or item.get(
                ...
                if not item.get(field_map.get(self.doctype)):
                    default_deferred_account = frappe.get_cached_value(
                        "Company",
                        self.company,
                        "default_" + field_map.get(self.doctype),
                    )
    ...
    def before_print(self, settings=None):
        if self.doctype in [
            ...
            if (
                ...
                and hasattr(self, "taxes")
                and not len(self.taxes)
            ):
                df.set("print_hide", 0)
    ...
    # 	from stock.controllers.taxes_and_totals import calculate_taxes_and_totals
    ...
    def set_missing_item_details(self, for_validate=False):
        ...
        from stock.stock.doctype.serial_no.serial_no import get_serial_nos
            ...
            for item in self.get("items"):
                if item.get("item_code"):
                        ...
                        if item.meta.get_field(fieldname) and value is not None:
                            if (
                                ...
                                item_conversion_factor = (
                                    item.get("conversion_factor") or 1.0
                                )
                    ...
                    if self.doctype in [
                        "Purchase Invoice",
                        "Sales Invoice",
                    ] and item.meta.get_field("is_fixed_asset"):
                    ...
                    if hasattr(item, "cost_center") and not item.get("cost_center"):
                        item.set(
                            ...
                            or stock.get_default_cost_center(self.company),
                        )
        ...
        if self.get("taxes_and_charges"):
            ...
            self.extend(
                ...
                get_taxes_and_charges(
                    tax_master_doctype, self.get("taxes_and_charges")
                ),
            )
        ...
        if self.taxes_and_charges and frappe.get_cached_value(
            ...
            frappe.throw(
                _("{0} '{1}' is disabled").format(
                    taxes_and_charges_doctype, self.taxes_and_charges
                )
            )
    ...
    def get_voucher_subtype(self):
        ...
        if self.doctype in voucher_subtypes:
            ...
            self.doctype == "Sales Invoice" and self.is_return
        ) or self.doctype == "Purchase Invoice":
            return "Credit Note"
        elif (
            self.doctype == "Purchase Invoice" and self.is_return
        ) or self.doctype == "Sales Invoice":
            return "Debit Note"
        return self.doctype
    ...
    def clear_unallocated_advances(self, childtype, parentfield):
        self.set(
            parentfield,
            self.get(parentfield, {"allocated_amount": ["not in", [0, None, ""]]}),
        )
    ...
    def validate_advance_entries(self):
        order_field = (
            "sales_order" if self.doctype == "Sales Invoice" else "purchase_order"
        )
        order_list = list(
            set(d.get(order_field) for d in self.get("items") if d.get(order_field))
        )
        ...
        if advance_entries:
            ...
            for d in advance_entries:
                if (
                    not advance_entries_against_si
                    or d.reference_name not in advance_entries_against_si
                ):
                    frappe.msgprint(
                        _(
    ...
    # 	from stock.controllers.status_updater import get_allowance_for
    ...
    def get_billing_reference_details(
        self, reference_names, reference_doctype, based_on
    ):
        return frappe._dict(
            frappe.get_all(
                reference_doctype,
    ...
    def set_total_advance_paid(self):
        ...
        if advance:
            ...
            if advance.account_currency == self.currency:
                ...
                precision = (
                    "base_rounded_total"
                    if self.get("base_rounded_total")
    ...
    def raise_missing_debit_credit_account_error(self, party_type, party):
        ...
        message += (
            "<br><ul><li>"
            + _("'Account' in the Accounting section of Customer {0}").format(
                link_to_party
            )
            + "</li>"
        )
        ...
        for adv in self.advances:
            ...
            if consider_for_total_advance:
                total_allocated_amount += flt(
                    adv.allocated_amount, adv.precision("allocated_amount")
                )
    ...
    def group_similar_items(self):
        ...
        for item in self.items:
            group_item_qty[item.item_code] = (
                group_item_qty.get(item.item_code, 0) + item.qty
            )
            group_item_amount[item.item_code] = (
                group_item_amount.get(item.item_code, 0) + item.amount
            )
        ...
        for item in self.items:
            if item.item_code in group_item_qty:
                ...
                if item.qty:
                    item.rate = flt(
                        flt(item.amount) / flt(item.qty), item.precision("rate")
                    )
                else:
                    item.rate = 0
    ...
    def validate_payment_schedule_dates(self):
        ...
        for d in self.get("payment_schedule"):
            if self.doctype == "Sales Order" and getdate(d.due_date) < getdate(
    ...
    def check_conversion_rate(self):
        default_currency = stock.get_company_currency(self.company)
    ...
    def repost_accounting_entries(self):
        if self.repost_required:
            ...
            repost_ledger.append(
                "vouchers", {"voucher_type": self.doctype, "voucher_no": self.name}
            )
...
def validate_inclusive_tax(tax, doc):
    ...
    if cint(getattr(tax, "included_in_print_rate", None)):
        if tax.charge_type == "Actual":
            ...
            [
                cint(t.included_in_print_rate)
                for t in doc.get("taxes")[: cint(tax.row_id) - 1]
            ]
        ):
...
def get_advance_journal_entries(
    ...
    if include_unallocated:
        q = q.where(
            (journal_acc.reference_name.isnull()) | (journal_acc.reference_name == "")
        )

    if order_list:
        q = q.where(
            (journal_acc.reference_type == order_doctype)
    ...
    if condition:
        ...
        if condition.get("name", None):
            common_filter_conditions.append(
                payment_entry.name.like(f"%{condition.get('name')}%")
            )

        if condition.get("from_payment_date"):
            common_filter_conditions.append(
                payment_entry.posting_date.gte(condition["from_payment_date"])
            )

        if condition.get("to_payment_date"):
            common_filter_conditions.append(
                payment_entry.posting_date.lte(condition["to_payment_date"])
            )

        if condition.get("get_payments") == True:
            if condition.get("cost_center"):
                common_filter_conditions.append(
                    payment_entry.cost_center == condition["cost_center"]
                )
            ...
            if condition.get("minimum_payment_amount"):
                common_filter_conditions.append(
                    payment_entry.unallocated_amount.gte(
                        condition["minimum_payment_amount"]
                    )
                )

            if condition.get("maximum_payment_amount"):
                common_filter_conditions.append(
                    payment_entry.unallocated_amount.lte(
                        condition["maximum_payment_amount"]
                    )
                )
...
def update_invoice_status():
    ...
    for doctype in ("Sales Invoice", "Purchase Invoice"):
        ...
        total_amount = (
            frappe.qb.terms.Case().when(consider_base_amount, base_total).else_(total)
        )
...
def get_payment_term_details(
    ...
    term_details.base_payment_amount = (
        flt(term.invoice_portion) * flt(base_grand_total) / 100
    )
...
def get_discount_date(term, posting_date=None, bill_date=None):
    ...
    if term.discount_validity_based_on == "Day(s) after invoice date":
        ...
        term.discount_validity_based_on == "Month(s) after the end of the invoice month"
    ):
...
def update_bin_on_delete(row, doctype):
    ...
    from stock.stock.stock_balance import (
        get_indented_qty,
        get_ordered_qty,
        get_reserved_qty,
        update_bin_qty,
...
def update_child_qty_rate(
    ...
    def validate_quantity(child_item, new_data):
        ...
        if parent_doctype == "Sales Order" and flt(new_data.get("qty")) < flt(
    ...
    items_added_or_removed = (
        False  # updated to true if any new item is added or removed
    )
    ...
    for d in data:
        ...
        if not d.get("docname"):
            ...
            prev_fg_qty, new_fg_qty = flt(child_item.get("fg_item_qty")), flt(
                d.get("fg_item_qty")
            )
        ...
        if flt(child_item.price_list_rate):
            if flt(child_item.rate) > flt(child_item.price_list_rate):
                ...
                child_item.discount_percentage = flt(
                    (1 - flt(child_item.rate) / flt(child_item.price_list_rate))
                    * 100.0,
                    child_item.precision("discount_percentage"),
                )
                child_item.discount_amount = flt(child_item.price_list_rate) - flt(
                    child_item.rate
                )
