...
frappe.ui.form.on("Purchase Receipt", {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_return === 1 && frm.doc.per_billed !== 100) {
			frm.add_custom_button(__('Debit Note'), function() {
				frappe.model.open_mapped_doc({
					method: "stock.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_invoice",
					frm: cur_frm,
				})
			}, __('Create'));
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_internal_supplier && !frm.doc.inter_company_reference) {
			frm.add_custom_button(__('Delivery Note'), function() {
				frappe.model.open_mapped_doc({
					method: 'stock.stock.doctype.purchase_receipt.purchase_receipt.make_inter_company_delivery_note',
					frm: cur_frm,
				})
			}, __('Create'));
		}
	...
	// 				method: "stock.accounts.doctype.purchase_invoice.purchase_invoice.make_purchase_receipt",
	...
	subcontracting_receipt: (frm) => {
		if (frm.doc.is_subcontracted === 1 && frm.doc.is_old_subcontracting_flow === 0 && frm.doc.subcontracting_receipt) {
			...
			stock.utils.map_current_doc({
				method: 'stock.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_purchase_receipt',
...
stock.stock.PurchaseReceiptController = class PurchaseReceiptController extends stock.buying.BuyingController {
	setup(doc) {
	...
	refresh() {
		...
		// stock.accounts.ledger_preview.show_accounting_ledger_preview(this.frm);
		// stock.accounts.ledger_preview.show_stock_ledger_preview(this.frm);
		...
		if (has_rejected_items && has_rejected_items.length > 0) {
			frappe.prompt([
				...
				if (values.return_for_rejected_warehouse) {
					frappe.call({
						method: "stock.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_return_against_rejected_warehouse",
						args: {
	...
	apply_putaway_rule() {
		if (this.frm.doc.apply_putaway_rule) stock.apply_putaway_rule(this.frm);
	}

};

// for backward compatibility: combine new and previous states
extend_cscript(cur_frm.cscript, new stock.stock.PurchaseReceiptController({frm: cur_frm}));

cur_frm.cscript.update_status = function(status) {
	frappe.ui.form.is_saving = true;
	frappe.call({
		method:"stock.stock.doctype.purchase_receipt.purchase_receipt.update_purchase_receipt_status",
...
cur_frm.cscript._make_purchase_return = function() {
	frappe.model.open_mapped_doc({
		method: "stock.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_return",
		frm: cur_frm
	});
}
...
cur_frm.cscript['Make Stock Entry'] = function() {
	frappe.model.open_mapped_doc({
		method: "stock.stock.doctype.purchase_receipt.purchase_receipt.make_stock_entry",
		frm: cur_frm,
	})
}
