...
import stock
from stock.controllers.item_variant import (
...
from stock.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from stock.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from stock.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from stock.stock.doctype.item_default.item_default import ItemDefault
		from stock.stock.doctype.item_reorder.item_reorder import ItemReorder
		from stock.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from stock.stock.doctype.item_tax.item_tax import ItemTax
		from stock.stock.doctype.item_variant_attribute.item_variant_attribute import (
			ItemVariantAttribute,
		)
		from stock.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": stock.get_default_currency(),
	...
	# 					barcode_type = convert_stock_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from stock.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"stock.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def convert_stock_to_barcodenumber(stock_number, barcode):
	if stock_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
	...
	if stock_number in convert:
		return convert[stock_number]
...
def get_asset_naming_series():
	from stock.assets.doctype.asset.asset import get_asset_naming_series
