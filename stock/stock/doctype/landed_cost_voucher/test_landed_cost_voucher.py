...
from stock.accounts.doctype.account.test_account import (
    create_account,
...
from stock.accounts.utils import update_gl_entries_after
from stock.assets.doctype.asset.test_asset import (
...
from stock.stock.serial_batch_bundle import SerialNoValuation
...
class TestLandedCostVoucher(FrappeTestCase):
    def test_landed_cost_voucher(self):
        ...
        self.assertEqual(
            last_sle.qty_after_transaction,
            last_sle_after_landed_cost.qty_after_transaction,
        )
        self.assertEqual(
            last_sle_after_landed_cost.stock_value - last_sle.stock_value, 25.0
        )
        ...
        update_gl_entries_after(
            pr.posting_date, pr.posting_time, warehouses, items, company=pr.company
        )
    ...
    def assertPurchaseReceiptLCVGLEntries(self, pr):
        ...
        for gle in gl_entries:
            if not gle.get("is_cancelled"):
                self.assertEqual(
                    expected_values[gle.account][0],
                    gle.debit,
                    msg=f"incorrect debit for {gle.account}",
                )
                self.assertEqual(
                    expected_values[gle.account][1],
                    gle.credit,
    ...
    def test_landed_cost_voucher_stock_impact(self):
        ...
        last_sle_after_landed_cost = (
            frappe.db.get_value(  # SLE of second PR after LCV's effect
                "Stock Ledger Entry",
                {
                    "voucher_type": pr2.doctype,
                    "voucher_no": pr2.name,
        ...
        self.assertEqual(
            last_sle_after_landed_cost.stock_value - last_sle.stock_value, 50.0
        )
    ...
    def test_landed_cost_voucher_against_purchase_invoice(self):
        ...
        self.assertTrue(gl_entries)
        stock_in_hand_account = get_inventory_account(
            pi.company, pi.get("items")[0].warehouse
        )
    ...
    def test_landed_cost_voucher_for_serialized_item(self):
        frappe.db.set_value(
            "Item", "_Test Serialized Item", "serial_no_series", "SNJJ.###"
        )
    ...
    def test_serialized_lcv_delivered(self):
        ...
        create_landed_cost_voucher(
            "Purchase Receipt", pr.name, pr.company, charges=charges
        )
    ...
    def test_landed_cost_voucher_for_odd_numbers(self):
        ...
        lcv = create_landed_cost_voucher(
            "Purchase Receipt", pr.name, pr.company, 123.22
        )
    ...
    def test_multi_currency_lcv(self):
        from basic.setup.doctype.currency_exchange.test_currency_exchange import (
            save_new_records,
            test_records,
        )
    ...
    def test_asset_lcv(self):
        ...
        self.assertEqual(
            frappe.db.get_value("Asset", assets[0].name, "gross_purchase_amount"), 50080
        )
        ...
        [
            {
                ...
                or "Expenses Included In Valuation - TCP1",
                "amount": args.charges,
            }
        ],
    )
...
def create_landed_cost_voucher(
    receipt_document_type, receipt_document, company, charges=50
):
...
def distribute_landed_cost_on_items(lcv):
    ...
    for item in lcv.get("items"):
        item.applicable_charges = (
            flt(item.get(based_on)) * flt(lcv.total_taxes_and_charges) / flt(total)
        )
        item.applicable_charges = flt(
            item.applicable_charges, lcv.precision("applicable_charges", item)
        )
