...
frappe.ui.form.on("Stock Reconciliation", {
	...
	scan_barcode: function(frm) {
		const barcode_scanner = new stock.utils.BarcodeScanner({frm:frm});
	...
	get_items: function(frm) {
		...
		frappe.prompt(fields, function(data) {
			frappe.call({
				method: "stock.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
				args: {
	...
	set_valuation_rate_and_qty: function(frm, cdt, cdn) {
		...
		if(d.item_code && d.warehouse) {
			frappe.call({
				method: "stock.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
	...
	// 		stock.is_perpetual_inventory_enabled(frm.doc.company));
	// },
	// purpose: function(frm) {
	// 	frm.trigger("set_expense_account");
	// },
	// set_expense_account: function(frm) {
	// 	if (frm.doc.company && stock.is_perpetual_inventory_enabled(frm.doc.company)) {
	// 		return frm.call({
	// 			method: "stock.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
	// 			args: {
...
frappe.ui.form.on("Stock Reconciliation Item", {
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		stock.utils.pick_serial_and_batch_bundle(frm, cdt, cdn, "Inward");
	}

});

stock.stock.StockReconciliation = class StockReconciliation extends stock.stock.StockController {
	setup() {
		var me = this;
	...
	refresh() {
		if(this.frm.doc.docstatus > 0) {
			...
			if (stock.is_perpetual_inventory_enabled(this.frm.doc.company)) {
...
cur_frm.cscript = new stock.stock.StockReconciliation({frm: cur_frm});
