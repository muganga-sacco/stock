...
//  stock.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");
...
frappe.ui.form.on('Stock Entry', {
	setup: function(frm) {
		...
		// stock.accounts.dimensions.setup_dimension_filters(frm, frm.doctype);
	...
	setup_quality_inspection: function(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(__("Quality Inspection(s)"), () => {
				let transaction_controller = new stock.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function(doc, cdt, cdn) {
			...
			return {
				query:"stock.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function(frm) {
		...
		if(frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Item'), () => {
					stock.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (frm.doc.add_to_transit && frm.doc.purpose=='Material Transfer' && frm.doc.per_transferred < 100) {
				frm.add_custom_button(__('End Transit'), function() {
					frappe.model.open_mapped_doc({
						method: "stock.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm
					})
				});
			}
		...
		// 		stock.utils.map_current_doc({
		// 			method: "stock.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
		...
		// 		const d = stock.utils.map_current_doc({
		// 			method: "stock.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus === 0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				frappe.call({
					method: "stock.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("items", []);
		...
		// 	stock.accounts.dimensions.update_dimension(frm, frm.doctype);
		// }
	...
	// 	frappe.call({
	// 		method: "stock.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
	// 	}).then(r => {
	// 		if (cint(r.message) == 0
	...
	make_retention_stock_entry: function(frm) {
		frappe.call({
			method: "stock.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args:{
	...
	// 	var enabled = stock.is_perpetual_inventory_enabled(frm.doc.company);
	...
	get_warehouse_details: function(frm, cdt, cdn) {
		...
		if(!child.bom_no) {
			frappe.call({
				method: "stock.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					"args": {
	...
	get_items_from_bom: function(frm) {
		...
		// 	frappe.call({
		// 		method: "stock.manufacturing.doctype.bom.bom.get_bom_items",
		// 		args: values,
		// 		callback: function(r) {
		// 			if (!r.message) {
		// 				frappe.throw(__("BOM does not contain any stock item"));
		// 			} else {
		// 				stock.utils.remove_empty_first_row(frm, "items");
	...
	source_warehouse_address: function(frm) {
		stock.utils.get_address_display(frm, 'source_warehouse_address', 'source_address_display', false);
	},

	target_warehouse_address: function(frm) {
		stock.utils.get_address_display(frm, 'target_warehouse_address', 'target_address_display', false);
	},
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) stock.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			stock.utils.map_current_doc({
				method: 'stock.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order',
...
frappe.ui.form.on('Stock Entry Detail', {
	...
	uom(doc, cdt, cdn) {
		...
		if(d.uom && d.item_code){
			return frappe.call({
				method: "stock.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty
				...
				callback: function(r) {
					if(r.message) {
						...
						if (no_batch_serial_number_value && !frappe.flags.hide_serial_batch_dialog && !frappe.flags.dialog_set) {
							...
							stock.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		stock.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},

	cost_center(frm, cdt, cdn) {
		stock.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		stock.stock.select_batch_and_serial_no(frm, child);
	}
});
...
var validate_sample_quantity = function(frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: 'stock.stock.doctype.stock_entry.stock_entry.validate_sample_quantity',
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
stock.stock.StockEntry = class StockEntry extends stock.stock.StockController {
	setup() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field('item_code').get_query = function() {
			return stock.queries.item({is_stock_item: 1});
		};
		...
		this.frm.set_query("supplier_address", stock.queries.address_query)
	}
	...
	refresh() {
		...
		// if (this.frm.doc.docstatus===1 && stock.is_perpetual_inventory_enabled(this.frm.doc.company)) {
		// 	this.show_general_ledger();
		// }
		stock.hide_company();
		stock.utils.add_item(this.frm);
	}
	...
	// 	if(this.frm.doc.company && stock.is_perpetual_inventory_enabled(this.frm.doc.company)) {
	// 		return this.frm.call({
	// 			method: "stock.accounts.utils.get_company_default",
	// 			args: {
	...
	// 					stock.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
	// 				}
	// 			}
	// 		});
	// 	}
	...
	work_order() {
		...
		return frappe.call({
			method: "stock.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new stock.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	}

	items_on_form_rendered(doc, grid_row) {
		stock.setup_serial_or_batch_no();
	}
	...
	supplier(doc) {
		stock.utils.get_party_details(this.frm, null, null, null);
	}
};

stock.stock.select_batch_and_serial_no = (frm, item) => {
...
function attach_bom_items(bom_no) {
	...
	frappe.db.get_doc("BOM",bom_no).then(bom => {
		...
		stock.stock.bom = {name, items:{}}
		items.forEach(item => {
			stock.stock.bom.items[item.item_code] = item;
		});
	});
}
...
function check_should_not_attach_bom_items(bom_no) {
  return (
	bom_no === undefined ||
	(stock.stock.bom && stock.stock.bom.name === bom_no)
  );
}

extend_cscript(cur_frm.cscript, new stock.stock.StockEntry({frm: cur_frm}));
