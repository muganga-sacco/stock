...
from hrms.setup.doctype.brand.brand import get_brand_defaults
...
from stock.stock.serial_batch_bundle import (
	SerialBatchCreation,
	get_empty_batches_based_work_order,
	get_serial_or_batch_items,
)
from stock.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from stock.stock.utils import get_bin, get_incoming_rate
...
class StockEntry(StockController):
	...
	if TYPE_CHECKING:
		...
		from stock.stock.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from stock.stock.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def before_validate(self):
		from stock.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate_item(self):
		...
		for item in self.get("items"):
			...
			update_fields = ("uom", "description", "cost_center", "conversion_factor")
	...
	def set_actual_qty(self):
		from stock.stock.stock_ledger import is_negative_stock_allowed
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				...
				d.basic_rate = get_valuation_rate(
					...
					currency=stock.get_company_currency(self.company),
					company=self.company,
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from stock.subcontracting.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from stock.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def get_items_from_subcontract_order(source_name, target_doc=None):
	from stock.controllers.subcontracting_controller import make_rm_stock_entry
