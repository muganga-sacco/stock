...
from stock.stock.doctype.item_manufacturer.item_manufacturer import (
...
def get_item_details(args, doc=None, for_validate=False, overwrite_warehouse=True):
    ...
    if doc:
        args["transaction_date"] = doc.get("transaction_date") or doc.get(
            "posting_date"
        )
    ...
    if (
        ...
        if doc and doc.get("doctype") == "Purchase Order":
            ...
            bin_details = get_bin_details(
                args.item_code, out.warehouse, include_child_warehouses=True
            )
...
def set_valuation_rate(out, args):
    if frappe.db.exists(
        "Product Bundle", {"name": args.item_code, "disabled": 0}, cache=True
    ):
        valuation_rate = 0.0
        ...
        for bundle_item in bundled_items.items:
            valuation_rate += flt(
                get_valuation_rate(
                    bundle_item.item_code, args.company, out.get("warehouse")
                ).get("valuation_rate")
                * bundle_item.qty
            )
        ...
        out.update(
            get_valuation_rate(args.item_code, args.company, out.get("warehouse"))
        )
...
def validate_item_details(args, item):
    ...
    from stock.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(args, item, overwrite_warehouse=True):
    ...
    if args.get("doctype") == "Purchase Invoice" and item.is_fixed_asset:
        from stock.assets.doctype.asset_category.asset_category import (
    ...
    if not args.get("uom"):
        if args.get("doctype") in sales_doctypes:
            ...
            args.get("doctype")
            in ["Purchase Order", "Purchase Receipt", "Purchase Invoice"]
        ) or (
            args.get("doctype") == "Material Request"
            and args.get("material_request_type") == "Purchase"
        ):
    ...
    out = frappe._dict(
        {
            ...
            "weight_per_unit": args.get("weight_per_unit")
    ...
    if args.get("manufacturer"):
        part_no = get_item_manufacturer_part_no(
            args.get("item_code"), args.get("manufacturer")
        )
        if part_no:
...
def get_item_warehouse(item, args, overwrite_warehouse, defaults=None):
    ...
    if not warehouse:
        ...
        if (
            frappe.db.get_value("Warehouse", default_warehouse, "company")
            == args.company
        ):
...
def get_item_tax_info(
    company, tax_category, item_codes, item_rates=None, item_tax_templates=None
):
    out = {}
...
def get_item_tax_template(args, item, out):
    ...
    if args.get("child_doctype") and item_tax_template:
        out.update(
            get_fetch_values(
                args.get("child_doctype"), "item_tax_template", item_tax_template
            )
        )
...
def _get_item_tax_template(args, taxes, out=None, for_validate=False):
    ...
    for tax in taxes:
        tax_company = frappe.get_cached_value(
            "Item Tax Template", tax.item_tax_template, "company"
        )
...
def calculate_service_end_date(args, item=None):
    ...
    service_start_date = (
        args.service_start_date if args.service_start_date else args.transaction_date
    )
...
def get_default_cost_center(args, item=None, item_group=None, brand=None, company=None):
    ...
    if args.get("project"):
        cost_center = frappe.db.get_value(
            "Project", args.get("project"), "cost_center", cache=True
        )
    ...
    if not cost_center and (item and item_group and brand):
        ...
        for method in [
            ...
            path = "stock.stock.get_item_details.{0}".format(method)
...
def get_price_list_rate(args, item_doc, out=None):
    ...
    if meta.get_field("currency") or args.get("currency"):
        ...
        if not out.price_list_rate and args.transaction_type == "buying":
            from stock.stock.doctype.item.item import get_last_purchase_details
...
def insert_item_price(args):
    ...
    if frappe.db.get_value(
        ...
        if frappe.has_permission("Item Price", "write"):
            price_list_rate = (
                (flt(args.rate) + flt(args.discount_amount))
                / args.get("conversion_factor")
...
def get_price_list_rate_for(args, item_code):
    ...
    if price_list_rate:
        ...
        if desired_qty and check_packing_list(
            price_list_rate[0][0], desired_qty, item_code
        ):
...
def get_party_item_code(args, item_doc, out):
    if args.transaction_type == "selling" and args.customer:
        ...
        customer_item_code = item_doc.get(
            "customer_items", {"customer_name": args.customer}
        )
        ...
        if customer_item_code:
            ...
            customer_group = frappe.get_cached_value(
                "Customer", args.customer, "customer_group"
            )
...
def get_conversion_factor(item_code, uom):
    ...
    conversion_factor = frappe.db.get_value(
        "UOM Conversion Detail", filters, "conversion_factor"
    )
...
def get_bin_details(item_code, warehouse, company=None, include_child_warehouses=False):
    ...
    if warehouse:
        ...
        from stock.stock.doctype.warehouse.warehouse import get_child_warehouses
...
def get_batch_qty(batch_no, warehouse, item_code):
    from stock.stock.doctype.batch import batch
...
def apply_price_list_on_item(args):
    item_doc = frappe.db.get_value(
        "Item", args.item_code, ["name", "variant_of"], as_dict=1
    )
