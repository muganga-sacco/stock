...
import stock
from stock.stock.doctype.bin.bin import update_qty as update_bin_qty
from stock.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
...
from stock.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from stock.controllers.stock_controller import future_sle_exists
...
class update_entries_after(object):
	...
	def update_serial_no_status(self, sle):
		from stock.stock.serial_batch_bundle import get_serial_nos
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		# Sales and Purchase Return
			...
			if frappe.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from stock.controllers.sales_and_purchase_return import (
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=stock.get_company_currency(sle.company),
			company=sle.company,
		)
...
def get_valuation_rate(
	...
	from stock.stock.serial_batch_bundle import BatchNoValuation
	...
	if (
		...
		and cint(stock.is_perpetual_inventory_enabled(company))
	):
