...
from stock.stock.get_item_details import get_default_bom
from stock.stock.stock_balance import get_reserved_qty, update_bin_qty
...
class SalesOrder(SellingController):
	...
	if TYPE_CHECKING:
		...
		from stock.selling.doctype.sales_order_item.sales_order_item import (
			SalesOrderItem,
		)
		from stock.selling.doctype.sales_team.sales_team import SalesTeam
		...
		naming_series: DF.Literal["REQ-.YYYY.-"]
	...
	def validate_sales_mntc_quotation(self):
		for d in self.get("items"):
			if d.prevdoc_docname:
				...
				if not res:
					frappe.msgprint(
						_("Quotation {0} not of type {1}").format(
							d.prevdoc_docname, self.order_type
						)
					)
	...
	def validate_delivery_date(self):
		if self.order_type == "Sales" and not self.skip_delivery_note:
			delivery_date_list = [
				d.delivery_date for d in self.get("items") if d.delivery_date
			]
			max_delivery_date = max(delivery_date_list) if delivery_date_list else None
			if (max_delivery_date and not self.delivery_date) or (
				max_delivery_date
				and getdate(self.delivery_date) != getdate(max_delivery_date)
			):
	...
	def validate_warehouse(self):
		...
		for d in self.get("items"):
			if (
				...
				frappe.throw(
					_("Delivery warehouse required for stock item {0}").format(
						d.item_code
					),
					WarehouseRequired,
				)
	...
	def validate_with_previous_doc(self):
		...
		if cint(
			frappe.db.get_single_value("Selling Settings", "maintain_same_sales_rate")
		):
	...
	def update_enquiry_status(self, prevdoc, flag):
		...
		if enq:
			frappe.db.sql(
				"update `tabOpportunity` set status = %s where name=%s",
				(flag, enq[0][0]),
			)
	...
	def validate_drop_ship(self):
		for d in self.get("items"):
			if d.delivered_by_supplier and not d.supplier:
				frappe.throw(
					_("Row #{0}: Set Supplier for item {1}").format(d.idx, d.item_code)
				)
	...
	def check_nextdoc_docstatus(self):
		...
		if linked_invoices:
			linked_invoices = [
				get_link_to_form("Sales Invoice", si) for si in linked_invoices
			]
			frappe.throw(
				_(
	...
	def check_modified_date(self):
		...
		date_diff = frappe.db.sql(
			"select TIMEDIFF('%s', '%s')" % (mod_db, cstr(self.modified))
		)
	...
	def assign_to_supervisor(self):
		...
		current_workflow = self.workflow_state
		
		if (current_workflow == 'Pending Supervisor Approval'):
			
			supervisor_email = self.assign_to
			
			# Assuming the task refers to the current document (self)
			if supervisor_email:
				email_status = send_notification_supervisor(supervisor_email, self)
				if email_status:
					frappe.msgprint(f"Notification sent to supervisor: {supervisor_email}")
				else:
	...
	def update_reserved_qty(self, so_item_rows=None):
		...
		for d in self.get("items"):
			if (
				not so_item_rows or d.name in so_item_rows
	...
	def update_delivery_status(self):
		...
		for item in self.items:
			if item.delivered_by_supplier:
				...
				item_delivered_qty = (
					item_delivered_qty[0][0] if item_delivered_qty else 0
				)
				item.db_set(
					"delivered_qty", flt(item_delivered_qty), update_modified=False
				)
	...
	def on_recurring(self, reference_doc, auto_repeat_doc):
		def _get_delivery_date(
			ref_doc_delivery_date, red_doc_transaction_date, transaction_date
		):
			delivery_date = auto_repeat_doc.get_next_schedule_date(
				schedule_date=ref_doc_delivery_date
			)

			if delivery_date <= transaction_date:
				delivery_date_diff = frappe.utils.date_diff(
					ref_doc_delivery_date, red_doc_transaction_date
				)
				delivery_date = frappe.utils.add_days(
					transaction_date, delivery_date_diff
				)
		...
		self.set(
			"delivery_date",
			_get_delivery_date(
				reference_doc.delivery_date,
				reference_doc.transaction_date,
	...
	def validate_serial_no_based_delivery(self):
		...
		for item in self.items:
			if item.ensure_delivery_based_on_produced_serial_no:
				...
				if item.item_code not in reserved_items:
					...
					if not frappe.db.exists(
						"BOM", {"item": item.item_code, "is_active": 1}
					):
						frappe.throw(
							_(
			...
			if (
				not item.ensure_delivery_based_on_produced_serial_no
				and item.item_code in reserved_items
			):
				frappe.throw(
					_(
						"Cannot ensure delivery by Serial No as Item {0} is added with and without Ensure Delivery by Serial No."
	...
	def validate_reserved_stock(self):
		...
		for item in self.items:
			if item.reserve_stock and (
				not enable_stock_reservation or not cint(item.is_stock_item)
			):
...
def get_list_context(context=None):
	from stock.controllers.website_list_for_contact import get_list_context
...
def get_requested_item_qty(sales_order):
	...
	for d in frappe.db.get_all(
		...
		result[d.sales_order_item] = frappe._dict(
	...
	if not kwargs.skip_item_mapping and kwargs.for_reserved_stock:
		...
		if sre_list:
			...
			for sre in sre_list:
				...
				dn_item.qty = flt(sre.reserved_qty) * flt(
					dn_item.get("conversion_factor", 1)
				)

				if sre.reservation_based_on == "Serial and Batch" and (
					sre.has_serial_no or sre.has_batch_no
				):
...
def create_pick_list(source_name, target_doc=None):
	from stock.stock.doctype.packed_item.packed_item import is_product_bundle
...
def send_notification_supervisor(user_email, notification):
	...
	message = f"""
		...
		<body>
			<center>
			<h2>Dear Supervisor,</h2>
			<p>You have been assigned Request: <strong>{notification.requester}</strong>.</p>
