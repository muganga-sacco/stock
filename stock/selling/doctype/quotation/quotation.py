...
class Quotation(SellingController):
    ...
    if TYPE_CHECKING:
        ...
        from basic.setup.doctype.quotation_lost_reason_detail.quotation_lost_reason_detail import (
            ...
            for reason in lost_reasons_list:
                if reason.get("lost_reason") in lost_reasons_lst:
                    ...
                    frappe.throw(
                        ...
                        ).format(frappe.bold(reason.get("lost_reason")))
                    )
...
def _make_sales_order(
    source_name, target_doc=None, customer_group=None, ignore_permissions=False
):
    ...
    if quotation and quotation.get("party_name"):
        if not frappe.db.exists("Customer", quotation.get("party_name")):
            ...
            if not customer_name:
                from stock.crm.doctype.lead.lead import _make_customer
